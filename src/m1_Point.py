...
def test_halfway_to():
    ...
    class Point(object):
        ...
        def move_to(self, x, y):
            self.travel += math.sqrt(((x - self.x) ** 2 + (y - self.y)) ** 2)
            self.x = x
            self.y = y
            self.number += 1
        ...
        def closer_to(self, p2, p3):
            self.distance_p2 = math.sqrt(((p2.x - self.x)) ** 2 + (p2.y - self.y) ** 2)
            self.distance_p3 = math.sqrt(((p3.x - self.x)) ** 2 + (p3.y - self.y) ** 2)
            if self.distance_p2 > self.distance_p3:
                return p3
            else:
                return p2
